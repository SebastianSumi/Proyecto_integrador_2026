...
| 4. Cabecera–detalle atómica, cálculos, estado, commit y rollback | `EvaluacionAptitud -> DetallePruebaFisica` ya es el agregado real: request compuesto, `cascade = ALL`, cálculo de puntaje/diagnóstico y servicio transaccional. La prueba de integración H2 verifica éxito con 1 cabecera + 2 detalles y un fallo posterior que revierte ambos. | La evidencia es local H2; aún falta repetirla contra Oracle autorizado durante la integración/demostración. Actividad–Inscripción no se presenta como cabecera–detalle. | **Aptitud Física/equipo:** preservar la operación compuesta y repetir la evidencia contra Oracle. |
...
| Cabecera–detalle | Ejecutar `EvaluacionAptitudTransactionalIntegrationTest`: registra 1 evaluación con 2 detalles y provoca una excepción posterior dentro de la transacción externa. | En H2, éxito persiste cabecera/detalles/cálculos; fallo deja ambos conteos en cero. Repetir contra Oracle autorizado. |
...
- Actividad–Inscripción **no** se llama cabecera–detalle; el agregado S06 real es `EvaluacionAptitud -> DetallePruebaFisica`, con evidencia de transacción local H2.
