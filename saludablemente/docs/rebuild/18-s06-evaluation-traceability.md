...
| 2. CRUD, validaciones, excepciones, logs y pruebas | CRUD local de Teams, Actividades y Metas; transiciones de Inscripción/Meta; 400/404/409 y suite registrada 118/118. | Logs transversales no están implementados/evidenciados; resultado Maven debe refrescarse antes de demo. | **Equipo de integración:** acordar logs. **Pedro:** ejecutar `mvn test` y demostrar CRUD válido + 400 en vivo. |
| 3. Objetos relacionados mediante ORM, DTO y reglas de asociación | IDs escalares y servicios públicos evitan acoplamiento; Inscripción valida Actividad mediante `ActividadService`. | No existe asociación ORM con DTO relacionado demostrable. | **Equipo/propietarios de dominio:** aprobar relación propia y DTO relacionado; implementar y probar navegación sin ciclos antes de atribuir cumplimiento. |
...
| Asociación ORM | Crear/consultar entidad relacionada mediante DTO de respuesta. | Navegación controlada sin ciclo de serialización. |
