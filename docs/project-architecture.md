...
El baseline Oracle local vigente está en `database/oracle/local/`, derivado del inventario JPA canónico. Los deltas concurrentes de Actividades están en `database/oracle/manual-migrations/` y se aplican manualmente con autorización. La FK de inscripción apunta a `ACTIVIDADES`; las referencias físicas a Personal y Auth están diferidas hasta acordar propietario, tabla, columna, tipo y grants.
